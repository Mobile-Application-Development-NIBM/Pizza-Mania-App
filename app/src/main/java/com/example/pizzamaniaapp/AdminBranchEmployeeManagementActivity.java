...
public class AdminBranchEmployeeManagementActivity extends AppCompatActivity { // main activity class for managing branches & employees
    ...
    private void showBranchPopup(Branch existingBranch, String preloadedBranchID) { // show popup to add or edit a branch
        ...
        addBtn.setOnClickListener(v -> { // handle save button click
            ...
            catch (NumberFormatException e) { Toast.makeText(this, "Invalid contact number", Toast.LENGTH_SHORT).show(); return; }

            // parse location values
            String[] locParts = locationStr.split(",");
            if (locParts.length != 2) { Toast.makeText(this, "Enter location as lat,long", Toast.LENGTH_SHORT).show(); return; }

            double latitude, longitude;
            try { latitude = Double.parseDouble(locParts[0].trim()); longitude = Double.parseDouble(locParts[1].trim()); }
            catch (NumberFormatException e) { Toast.makeText(this, "Invalid location values", Toast.LENGTH_SHORT).show(); return; }
            ...
            db.child("branches").child(id).setValue(branch) // save to Firebase
                    .addOnSuccessListener(aVoid -> { Toast.makeText(this, "Branch saved!", Toast.LENGTH_SHORT).show(); loadBranches(); dialog.dismiss(); }) // success
                    .addOnFailureListener(e -> Toast.makeText(this, "Failed: " + e.getMessage(), Toast.LENGTH_SHORT).show()); // failure
        });
    }
    ...
    private void showAddEmployeePopup(String branchId) { // open popup to add new employee
        ...
        addBtn.setOnClickListener(v -> { // save employee button clicked
            ...
            if(empID.isEmpty() || userID.isEmpty() || name.isEmpty() || email.isEmpty() || contactStr.isEmpty() || address.isEmpty()){ // check empty fields
                Toast.makeText(this,"Please fill all fields",Toast.LENGTH_SHORT).show(); // show warning
                return; // stop execution
            }

            long contact;
            try { contact = Long.parseLong(contactStr); } // parse contact to number
            catch(NumberFormatException e){ Toast.makeText(this,"Invalid contact",Toast.LENGTH_SHORT).show(); return; } // handle invalid number
            ...
            db.child("employees").child(empID).setValue(employee) // save employee to Firebase
                    .addOnSuccessListener(aVoid -> db.child("users").child(userID).setValue(user) // save user on success
                            .addOnSuccessListener(aVoid2 -> { Toast.makeText(this,"Employee added!",Toast.LENGTH_SHORT).show(); loadBranches(); dialog.dismiss(); }) // success message
                            .addOnFailureListener(e -> Toast.makeText(this,"Failed to add user: "+e.getMessage(),Toast.LENGTH_SHORT).show())) // handle user save failure
                    .addOnFailureListener(e -> Toast.makeText(this,"Failed to add employee: "+e.getMessage(),Toast.LENGTH_SHORT).show()); // handle employee save failure
        });
    }
    ...
    private void showEditEmployeePopup(Employee employee) { // open popup to edit an existing employee
        ...
        addBtn.setOnClickListener(v -> { // update employee on click
            ...
            catch(NumberFormatException e){ Toast.makeText(this,"Invalid contact",Toast.LENGTH_SHORT).show(); return; }
            ...
            db.child("employees").child(employee.employeeID).setValue(employee) // save updated employee to DB
                    .addOnSuccessListener(aVoid -> {
                        ...
                        Toast.makeText(this,"Employee updated!",Toast.LENGTH_SHORT).show(); // success message
                        loadBranches(); dialog.dismiss(); // reload branches and dismiss dialog
                    })
                    .addOnFailureListener(e -> Toast.makeText(this,"Update failed: "+e.getMessage(),Toast.LENGTH_SHORT).show()); // failure message
        });
    }
    ...
    private void showDeleteConfirmation(String type, String key) { // open popup to confirm deletion
        ...
        confirmBtn.setOnClickListener(v -> { // handle confirm click
            if(type.equals("branch")) db.child("branches").child(key).removeValue().addOnSuccessListener(aVoid -> loadBranches()); // delete branch and reload
